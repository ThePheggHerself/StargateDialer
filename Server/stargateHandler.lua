...
function listenStargateIncomingWormhole() -- "stargate_incoming_wormhole"
	while true do
		...
		if addrStr == nil then
			...
			CreateDisplayLink.UpdateDisplay({ content = "Incoming Wormhole", xPos = 7 },
				{ content = "Origin Unavailable", xPos = 7 })
			...
			if address.id ~= nil then
				...
				CreateDisplayLink.UpdateDisplay({ content = "Incoming Wormhole", xPos = 7 },
					{ content = address.display, xPos = 7 })
				...
				CreateDisplayLink.UpdateDisplay({ content = "Incoming Wormhole", xPos = 7 },
					{ content = address.address, xPos = 7 })
...
function isAdvancedInterface(interfaceString)
	if interfaceString ~= nil and string.find(interfaceString, "advanced_crystal_interface") then
		return true
	else
		return false
	end
